...
public class MultipleLoginSet2 {
	
	WebDriver driver;
	XSSFWorkbook wb;
	XSSFWorkbook wb1;

	@Test(description="This example for Empty Excel File")
	public void f() throws Exception{
		...
		for(int i=1; i<rowCount+1; i++){
           ...
           sheet1.createRow(i).createCell(2).setCellValue(Result);   //here we are creating row and set the value for empty excel file
			
			Row row=sheet.getRow(i);
			int colCount=row.getLastCellNum();
			System.out.println("Total Column Number: "+ colCount);
			for(int j=0; j<row.getLastCellNum(); j++)        //here -1 is removed because in this example sheet column number is not available
			{
		...
		sheet1.createRow(0).createCell(0).setCellValue("UserName");    //Only first time row will be get created
		sheet1.getRow(0).createCell(1).setCellValue("Password");      // second time only cell will be created
